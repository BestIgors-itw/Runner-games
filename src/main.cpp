...
int main() {
	...
	int object_generate_probability = 25000;
	float bk_timer = background_timer.getElapsedTime().asMicroseconds();
	...
	while (window.isOpen()){
		...
		object_generate_probability = object_generate_probability - bk_timer - rand() % 500;
		...
		if (object_generate_probability < 0) {
			...
			object_generate_probability = 25000;
			background_timer.restart();
			bk_timer = background_timer.getElapsedTime().asMicroseconds();
		}
